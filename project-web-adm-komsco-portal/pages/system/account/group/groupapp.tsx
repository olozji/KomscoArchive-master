...
import { ISelectAccountGroupApp } from '@/types/Account';
...
const GroupApp: NextPageWithLayout = () => {
  const [groupAppList, setGroupAppList] = useState<ISelectAccountGroupApp>();
  ...
  const handleParams = (name: string, _value: any) => {
    setGroupAppList({ ...groupAppList, [name]: _value });
  };
  ...
  const handleLnknSiteList = (useCheck: boolean, _value: string) => {
    let _mngrGroupSiteList = groupAppList.mngrGroupSiteList;
    let returnUse = false;
    if (!useCheck) {
    ...
    setGroupAppList({ ...groupAppList, mngrGroupSiteList: _mngrGroupSiteList });
  ...
  const updateGroupApp = () => {
    const intArray = StrArrayToInt(menuCheckList);
    const { mngrGroupSeq, mngrGroupNm, uzYn, acntTypCd, authTypCd } = groupAppList;
    const params = {};
      ...
      <section>
        ...
        <Wrap>
          <BodyBox title="관리자그룹 명">
            <FormControl
              defaultValue={groupAppList?.mngrGroupNm}
              changeableValue={groupAppList?.mngrGroupNm}
          ...
          <BodyBox title="계정유형">{groupAppList?.acntTypNm}</BodyBox>
...
GroupApp.getLayout = (page, navItems) => {
  return (
