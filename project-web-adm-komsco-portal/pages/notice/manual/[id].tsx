...
const addMenual = () => {
  ...
  const putData = () => {
    ...
    if (detailData?.mnulSeq != null) {
      NoticeMng.putMenual(detailData?.mnulSeq, {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (detailData != null) {
        if (mgmtRef.current != null) {
          //관리 시스템
          mgmtRef.current.value = detailData?.mgmtSysCd;

          //계정유형
          for (let i = 0; i < mgmtSysCdRef?.current?.length; i++) {
            for (let j = 0; j < detailData?.acntTypCds?.length; j++) {
              if (mgmtSysCdRef?.current[i]?.value === detailData?.acntTypCds[j]) {
          ...
          if (checked) {
            mgmtSysAllRef.current.checked = true;
          }
          ...
          //메뉴얼명
          mnulNmRef.current.value = detailData?.mnulNm;

          //설명
          mnulDescRef.current.value = detailData?.mnulDesc;
  ...
  const getData = () => {
    ...
    console.log('idData', idData[idData.length - 2]);
    if (idData != null) {
      NoticeMng.detailMenual(idData[idData.length - 2]).then((response: any) => {
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 230, maxWidth: 600 }} aria-label="customized table">
                <TableBody>
                  ...
                  <div>
                    {detailData?.hisList.map((history, index) => {
                      return (
                        <TopStyledTableRow
                          sx={{
                            display: 'grid',
