...
const rpolicySttst: NextPageWithLayout = () => {
  ...
  const [params, setParams] = useState({
    ...
    schGftcDivCd: '02', // 상품권/정책수당 선택 코드
  ...
  const fetchPolicyRefundPayment = async () => {
    console.log('정책수당 회수 통계');
    ...
    GiftPolicyService.PolicyRefundPayment(params).then((response: any) => {
      const data = response;
  ...
  const fetchGiftValue = async () => {
    CommonService.getCodeList({
      schCdDvcd: '1122', // 코드 정의서에 있는 1122(정책수당구분코드)
    })
      .then((response: any) => {
        console.log('-----코드 리스트0---');
        ...
        setpolicyDetailList(response.body.list);
      })
      .catch((e: any) => {
        console.log(e);
      });
  ...
  const getGiftCodeData = async () => {
    console.log('내가 선택한 지역');
    console.log(params.schAreaSrvcUid);
          ...
          <Autocomplete
            ...
            onChange={(event, value) => {
              if (value == null) {
                ...
                setParams({ ...params, schAreaSrvcUid: value.value, schGftcDivCd: '02' });
                getGiftCodeData();
                ...
                {items.map((row) => (
                  <StyledTableRow key={row.id}>
                    <StyledTableCell align="center">
                      {dateFormat(params.schTrmSrchDiv, row.rtrvDt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.areaSrvcNm)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.plcyAlowNm)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.mngrPartRtrvCcnt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.mngrPartRtrvAmt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.mngrEnamRtrvCcnt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.mngrEnamRtrvAmt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.secssRtrvCcnt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.secssRtrvAmt)}
                    </StyledTableCell>

                    <StyledTableCell align="center">
                      {numberWithCommas(row.vldrngExprnRtrvCcnt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.vldrngExprnRtrvAmt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.totRtrvCcnt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.totRtrvAmt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
                      {numberWithCommas(row.afrtBalAmt)}
                    </StyledTableCell>
                  </StyledTableRow>
                ))}
              </TableBody>
...
export default rpolicySttst;

rpolicySttst.getLayout = (page, navItems) => {
  return (
    <PrimaryLayout title={'정책수당 회수통계'} navItems={navItems}>
      {page}
    </PrimaryLayout>
  );
};
