...
const cancelSttst: NextPageWithLayout = () => {
  ...
  const fetchCardCancelPayment = async () => {
    console.log('상품권 구매취소 조회');
    ...
    GiftPolicyService.GiftCancelPayment(params).then((response: any) => {
      const data = response;
          ...
          <Table sx={{ minWidth: 800 }} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell align="center">구매취소 금액</StyledTableCell>
                <StyledTableCell align="center">실 환불 금액</StyledTableCell>
              ...
              <TableBody>
                {items.map((row) => (
                  <StyledTableRow key={row.id}>
                    ...
                    <StyledTableCell align="center">
                      {numberWithCommas(row.buynCancAmt)}
                    </StyledTableCell>
                    <StyledTableCell align="center">
...
export default cancelSttst;

cancelSttst.getLayout = (page, navItems) => {
  return (
    <PrimaryLayout title={'상품권 구매취소 통계'} navItems={navItems}>
      {page}
    </PrimaryLayout>
  );
};
